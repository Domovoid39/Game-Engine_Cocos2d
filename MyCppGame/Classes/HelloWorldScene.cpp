...
void HelloWorld::onMouseMove(cocos2d::Event* _event)
{
	
}
void HelloWorld::onMouseUp(cocos2d::Event* _event)
{

}
void HelloWorld::onMouseDown(cocos2d::Event* _event)
{
	EventMouse *e = (EventMouse*)_event;
	Vec2 touchPoint = Vec2(e->getCursorX(), e->getCursorY());
	CheckGemTouch(touchPoint);
}
void HelloWorld::onMouseScroll(cocos2d::Event* _event)
{
...
void HelloWorld::CheckEntireArray(void)
{
	for (int i = 0; i < MAX_HORIZONTAL; ++i)
		for (int j = 0; j < MAX_VERTICAL; ++j) {
			if (!CheckLowerX(i, j)) {
				...
				m_GSM->setGameState(E_GAME_REMOVE);
			}
			if (!CheckLowerY(i, j)) {
				for (int k = 0; k < 3; k++) {
...
void HelloWorld::RemoveGems(void)
{
	for (int i = 0; i < MAX_HORIZONTAL; ++i)
		for (int j = 0; j < MAX_VERTICAL; ++j)
	...
	m_GSM->setGameState(E_GAME_CREATE);
}

void HelloWorld::ShiftGems(void)
{
	USHORT shCur;
	for (int i = 0; i < MAX_HORIZONTAL; ++i) {
		shCur = MAX_VERTICAL;
...
void HelloWorld::CreateGems(void)
{
	for (int i = 0; i < MAX_HORIZONTAL; ++i)
		for (int j = 0; j < MAX_VERTICAL; ++j)
			if (m_arrGem[i][j].gem->getState() == E_GEM_REMOVE) {
	...
	//ShiftGems();
	m_GSM->setGameState(E_GAME_REST);
}

bool HelloWorld::CheckLowerX(USHORT x, USHORT y)
{
	if (x < 2)
		return true;
	for (int i  = 1; i < 3; i++)
...
bool HelloWorld::CheckLowerY(USHORT x, USHORT y )
{
	if (y < 2) return true;
	for (int i  = 1; i < 3; i++)
	{
...
void HelloWorld::CheckGemTouch(Vec2 touchPoint)
{
	if (m_GSM->getGameState() != E_GAME_NORMAL)
		return;
...
void HelloWorld::ChangeGemTexture(void)
{
	/*E_GEM_TYPES eGemType = sNodeSelected2->gem->getType();
	sNodeSelected2->gem->ChangeTexture(sNodeSelected1->gem->getType());
	sNodeSelected1->gem->ChangeTexture(eGemType);*/
...
bool HelloWorld::CheckSingleGem(USHORT x, USHORT y)
{
	for (int i = 0; i < MAX_HORIZONTAL; i++)
	{
		if (!CheckLowerX(i, y))
